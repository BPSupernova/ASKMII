...
TestHero::TestHero() {
    ...
    // Player controls TestHero, so register for input events.
    ...
    df::Vector2D p(7, WM.getBoundary().getVertical() / 2);
    setPosition(p);
...
TestHero::~TestHero() {
    ...
    for (int i = -8; i <= 8; i += 5) {
        for (int j = -5; j <= 5; j += 3) {
            df::Vector2D temp_pos = this->getPosition();
...
void TestHero::mouse(const df::EventMouse* p_mouse_event) {
    ...
    if ((p_mouse_event->getMouseAction() == df::CLICKED) &&
        ...
        fire(p_mouse_event->getMouseCursorPos());
}
...
void TestHero::move(int dy) {
    ...
    df::Vector2D new_pos(getPosition().getX(), getPosition().getY() + dy);
    if ((new_pos.getY() > 3) &&
...
void TestHero::fire(df::Vector2D target) {
    LM.getInstance().writeLog("target position: %f, %f\n", target.getX(), target.getY());
    // See if time to fire.
    if (fire_countdown > 0)
        return;
    fire_countdown = fire_slowdown;
    ...
    p->setVelocity(df::Vector2D(p->getVelocity().getX(),
